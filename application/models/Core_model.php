...
class Core_model extends CI_Model
{
    ...
    public function inserir($tabela = NULL, $data = NULL, $ultimo_id = NULL) 
    {
           if($tabela && is_array($data))
           {
               ...
               if($ultimo_id)
               {
                   $this->session->set_userdata('ultimo_id', $this->db->insert_id());
               }
               ...
               {
                   $this->session->set_flashdata('error','Erro ao inserir dados!') ;
               }
           }
    }
    
    public function atualizar($tabela = NULL, $data = NULL, $condicao = NULL) 
    {
    ...
    public function apagar($tabela = NULL, $condicao = NULL) 
    {
            ...
            if($tabela && is_array($condicao))
            {
                   ...
                   if(!$status)
                   {
                       foreach ($error as $code) 
                       {
                               if($code == 1451) 
                               {
                                       $this->session->set_flashdata('Esse registro não pode ser apagado pois está em uso em outra tabela') ;
                               }
                    ...
                    {
                       $this->session->set_flashdata('Registro excluído com sucesso"');
                    }
    ...
    //função para gerar matrícula do vendedor
    public function gerar_codigo_unico($table = NULL, $type_of_code = NULL, $size_of_code, $field_search) 
    {
            do 
            {
